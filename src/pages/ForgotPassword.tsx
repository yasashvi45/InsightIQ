        ...
        <AnimatePresence mode="wait">
          {!isSent ? (
            ...
            >
              ...
              <h1 className="text-2xl font-heading font-bold text-[var(--color-brand-text)] mb-2">Forgot Password?</h1>
              <p className="text-[var(--color-brand-muted)] text-sm mb-8 leading-relaxed">No worries, we'll send you reset instructions.</p>
              ...
              <form onSubmit={handleReset} className="space-y-6">
                ...
                <div>
                  ...
                  <input 
                    ...
                    className="w-full px-4 py-3 bg-[var(--color-brand-bg)] border border-[var(--color-brand-border)] rounded-xl text-[var(--color-brand-text)] placeholder-[var(--color-brand-muted)] focus:outline-none focus:border-[var(--color-brand-primary)] focus:ring-1 focus:ring-[var(--color-brand-primary)] transition-all"
                    disabled={isLoading}
                  />
                </div>
                <button 
            ...
            >
              ...
              <h1 className="text-2xl font-heading font-bold text-[var(--color-brand-text)] mb-4">Check your email</h1>
              <p className="text-[var(--color-brand-muted)] text-sm mb-8 leading-relaxed">
                We sent a password reset link to <br/> <span className="font-medium text-[var(--color-brand-text)]">{email}</span>
              </p>
              
              <button 
                onClick={() => setIsSent(false)} 
                className="w-full py-3 bg-[var(--color-brand-bg)] border border-[var(--color-brand-border)] text-[var(--color-brand-text)] font-medium rounded-xl hover:border-[var(--color-brand-muted)] hover:bg-[var(--color-brand-card)] transition-colors mb-4 active:scale-[0.98]"
              >
                Didn't receive the email? Click to resend
              </button>

              <div className="pt-4 mt-4 border-t border-[var(--color-brand-border)] border-dashed">
                <p className="text-xs text-[var(--color-brand-muted)] mb-3 flex items-center justify-center gap-2">
                  <Sparkles className="w-3 h-3" /> Dev Mode Shortcut
                </p>
                <button 
                  onClick={() => navigate('/reset-password')}
                  className="text-xs font-medium text-blue-400 hover:text-blue-300 transition-colors"
                >
                  [Mock Email Link] Go to Reset Password
                </button>
              </div>
            </motion.div>
          )}
